...
score_range::score_range(const size_t& num_lists) : worst_score_(0), best_score_(0), num_lists_(num_lists) {
	discovered_fields_ = new double[num_lists_];
	std::fill_n(discovered_fields_, num_lists_, -1);
}
...
bool   score_range::DiscoveredOn (const size_t& index) const { return discovered_fields_[index] >= 0; }
